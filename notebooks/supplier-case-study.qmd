...
task_train <- mlr3::as_task_classif(train_data, target = "result", id = "supplier_id")
task_test <- mlr3::as_task_classif(test_data, target = "result", id = "supplier_id")
task <- mlr3::as_task_classif(all_data, target = "result", id = "supplier_id")
```
...
```{r}
measures <- mlr3::msrs(c("classif.acc", "classif.ce", "classif.logloss"))
...
```{r}
#| label: rolling-window-cv
#| warning: false
library(mlr3temporal)

resampling <- rsmp("RollingWindowCV", fixed_window = FALSE, horizon = 1, folds = 10)
